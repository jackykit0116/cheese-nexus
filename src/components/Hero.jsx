  ...
  return (
    <section className="relative flex flex-col items-center justify-center py-24 px-6 overflow-hidden min-h-screen bg-gradient-to-b from-[#050505] to-[#0a0a0a]">
      ...
      >
        ...
        {/* Intent Capture Interface - Implicit Operations Layer */}
        <motion.div
        ...
        >
          <div className="bg-black/30 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
            <div className="text-center mb-4">
              <p className="text-sm text-gray-400 mb-2">工作流主導的智能體時代</p>
              <p className="text-xs text-gray-500">告訴我你需要什麼，我會自動完成剩餘步驟</p>
            </div>

            <textarea
              placeholder="例如：幫我準備明天的會議簡報"
              className="w-full bg-black/50 border border-white/10 rounded-xl p-4 text-white placeholder-gray-600 focus:border-red-500/50 focus:ring-2 focus:ring-red-500/20 outline-none transition-all resize-none"
              rows={2}
            />
            ...
            >
              <p className="text-xs text-gray-500 font-mono">自動執行流程：</p>
              <div className="grid grid-cols-4 gap-2">
                {[
                  { label: '分析需求', status: 'completed', icon: '✓' },
                  { label: '分解任務', status: 'completed', icon: '✓' },
                  { label: '選擇工具', status: 'completed', icon: '✓' },
                  { label: '生成輸出', status: 'pending', icon: '⏳' },
                ].map((step, i) => (
                  <div key={i} className={`flex items-center gap-2 px-2 py-1 rounded-lg ${
                    step.status === 'completed' ? 'bg-green-500/10' : 'bg-gray-500/10'
                  }`}>
                    <span className={`text-xs font-mono ${step.status === 'completed' ? 'text-green-500' : 'text-gray-500'}`}>
                      {step.icon}
                    </span>
            ...
            <Button 
              size="lg" 
              className="w-full mt-4 bg-gradient-to-r from-red-600 to-orange-600 text-white font-bold shadow-[0_0_30px_rgba(255,50,0,0.4)] hover:shadow-[0_0_50px_rgba(255,50,0,0.6)] transition-all hover:scale-105"
